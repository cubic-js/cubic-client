...
class Auth {
  ...
  async getToken () {
    ...
    try {
      ...
      let timeout = err.error.reason ? parseInt(err.error.reason.replace(/[^0-9]+/g, '')) : 5000
      await timeout(() => this.getToken(this.options.user_key, this.options.user_secret), timeout)
    }
  }
